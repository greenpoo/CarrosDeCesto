...
public class PhysicsActor extends Actor {
	private Vector2D _r, _v, _a, _f, _ibounds = null, _hbounds = null;
	...
	public void setScale(double scale) {
		_hbounds = _ibounds.scale(scale / 2);
	}
	...
	protected final void collideWithWalls(Vector2D mapSize) {
		...
		if ((dr = _r.getX() - _hbounds.getX()) < 0 ||
				(dr = _r.getX() - _hbounds.getX() + mapSize.getX()) > 0) {
			_v = _v.scale(-1, 1);
			_r = _r.add(-dr, 0);
		}

		if ((dr = _r.getY() - _hbounds.getY()) < 0 ||
				(dr = _r.getY() - _hbounds.getY() + mapSize.getY()) > 0) {
			_v = _v.scale(1, -1);
			_r = _r.add(0, -dr);
		}
	}
